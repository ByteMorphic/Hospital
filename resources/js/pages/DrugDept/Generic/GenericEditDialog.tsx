...
export interface Generic {
  id: number;
  generics_name: string;
  generics_description: string;
...
interface WardEditDialogProps {
  isOpen: boolean;
  onClose: () => void;
  generic?: Generic; // Mark as optional if it might be undefined
}

export function WardEditDialog({ isOpen, onClose, generic }: WardEditDialogProps) {
  const [formSubmitted, setFormSubmitted] = useState(false);
  
  const form = useForm({
    ward_name: generic?.ward_name || '',
    ward_description: generic?.ward_description || '',
    ward_capacity: generic?.ward_capacity || 0,
    ward_status: generic?.ward_status ?? true
  });
